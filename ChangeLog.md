...
- Оновлено `backend/tests/watchlist/test_watchlist_api.py` і `frontend/tests/app.test.tsx`: додано regression coverage, що repeated `GET /watchlist` не з’їдає update marker, explicit acknowledge гасить лише unread state, а frontend після `Mark seen` прибирає badge `Changed`, але не ховає текст про останню зміну.
- Verification: `./.venv/bin/pytest backend/tests/watchlist/test_watchlist_api.py` -> `12 passed` (є лише `urllib3` `LibreSSL` warning у локальному Python runtime); `cd frontend && npm test -- app.test.tsx` -> `55 passed`; `cd frontend && npm run build` -> успішно.
...
## [2026-04-01 15:09] Add backend admin command-center APIs and managed user lifecycle
- Оновлено `backend/src/cartrap/modules/auth/{models.py,schemas.py,repository.py,service.py}` і `backend/tests/auth/{test_login.py,test_rbac.py}`: додано lifecycle statuses `blocked/disabled`, inactive-check для login/refresh/current-user flow, admin-safe user serialization і regression coverage на blocked login/token access.
- Додано `backend/src/cartrap/modules/admin/{schemas.py,service.py}` та розширено `backend/src/cartrap/modules/admin/router.py`: піднято `GET /api/admin/{overview,system-health,invites,users,users/{user_id}}` і `POST /api/admin/users/{user_id}/actions/{action}` з aggregate payload-ами для overview, user directory, detail inspector і root-mode actions.
...
## [2026-04-01 16:39] Implement admin runtime settings end-to-end
- Додано `backend/src/cartrap/modules/runtime_settings/{models.py,repository.py,schemas.py,service.py}` і розширено `backend/src/cartrap/config.py`: з’явився allowlisted Mongo-backed runtime settings overlay для safe operational knobs (`poll intervals`, `stale window`, `auction reminder offsets`, `job retry backoff`, `invite TTL`) з audit-полями, bounds validation і fallback до `.env` defaults.
- Оновлено `backend/src/cartrap/{app.py,api/dependencies.py,api/system.py,worker/main.py}` та `backend/src/cartrap/modules/{auth/admin/search/watchlist/monitoring/system_status}/*`: effective runtime values тепер реально wired у request-scoped services і worker cycle construction, тому admin overrides впливають на `/api/system/status`, invite TTL, freshness logic, reminder scheduling і job retry backoff без deploy/restart.
- Оновлено `backend/src/cartrap/modules/admin/{router.py,schemas.py,service.py}`: додано `GET /api/admin/runtime-settings`, `POST /api/admin/runtime-settings` і `POST /api/admin/runtime-settings/reset` з grouped metadata/effective/default/override payload-ами і atomic validation для bulk updates.
- Додано `frontend/src/features/admin/AdminRuntimeSettingsPanel.tsx` і оновлено `frontend/src/{App.tsx,lib/api.ts,types.ts,styles.css}`: в admin workspace з’явився `Runtime Settings` panel із save/reload/reset flows, bounds-aware editing, dirty-state UX і guard-ом, що non-admin bootstrap не викликає runtime-settings endpoints.
