...
def _extract_lot_payload(html: str) -> dict[str, Any]:
    ...
    for script_id in ("lot-details-data", "__NEXT_DATA__", "__NUXT_DATA__"):
        json_script = soup.find("script", attrs={"id": script_id})
        if json_script and json_script.string:
            return _extract_lot_from_json(json.loads(json_script.string))

    for pattern in LOT_JSON_PATTERNS:
        match = pattern.search(html)
        if match:
            return _extract_lot_from_json(json.loads(match.group(1)))

    if _is_challenge_page(html):
        raise CopartParseError("Copart returned an anti-bot challenge page instead of lot details.")
    ...
    logger.warning("Copart lot payload not found. title=%s script_ids=%s", title, script_ids[:10])
...
def _extract_lot_from_json(data: Any) -> dict[str, Any]:
    if isinstance(data, dict):
        if _looks_like_lot_dict(data):
            return data
        for value in data.values():
            try:
                return _extract_lot_from_json(value)
            except CopartParseError:
                continue
        raise CopartParseError("Could not locate lot payload in JSON object.")

    if isinstance(data, list):
        for item in data:
            try:
                return _extract_lot_from_json(item)
            except CopartParseError:
                continue
        raise CopartParseError("Could not locate lot payload in JSON list.")

    raise CopartParseError("Unsupported lot payload type.")
...
def _looks_like_lot_dict(value: dict[str, Any]) -> bool:
    keys = set(value.keys())
    return "lotNumber" in keys and "url" in keys and ("status" in keys or "saleStatus" in keys)
